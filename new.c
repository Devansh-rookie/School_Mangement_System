...
void add_student(std arr[])
{
    ...
    for(int i=0;i<5;i++)
	{
		// 5 Courses is the max taken by a student
...
void delete_student_roll(std arr[])
{
    if(n==0)
	{
		printf("There is no student in system");
    }
	else
	{
        long int key;
...
void encryption(){
	...
	fpRead=fopen("student_database.txt", "r");
	if(fpRead==NULL)
	{
...
void decryption(){
	...
	printf(" Input the name of file to name after decryption : ");
	scanf("%s",fname);	
